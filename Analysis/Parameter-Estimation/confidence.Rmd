...
# Expresión analítica de la pdf (para título del gráfico de densidad)
pdf_expression <- function(result) {
  switch(result$dist_id,
         "uniform" = expression(f(x) == frac(1, b - a) ~~ ",", ~~ a <= x ~~ "," ~~ x <= b),
         "weibull" = expression(f(x) == frac(k, lambda) ~~ (x / lambda)^{k - 1} ~~ e^{- (x / lambda)^k}),
         "lognormal" = expression(f(x) == frac(1, x ~ sigma ~ sqrt(2 * pi)) ~~ e^{-((log(x) - mu)^2) / (2 * sigma^2)}),
         "gamma" = expression(f(x) == frac(lambda^alpha, Gamma(alpha)) ~~ x^{alpha - 1} ~~ e^{-lambda * x}),
         expression(f(x)))
}
...
plot_true_distribution <- function(result, probs = c(0.001, 0.999)) {
  ...
  main_expr <- pdf_expression(result)
  ...
  axis(1,
       at = result$mean_true,
       labels = bquote(mu == .(round(result$mean_true, 3))),
       col.axis = "red",
       col = "red",
       tick = TRUE)
  legend("topright",
         legend = c(expression(f(x)), expression(mu)),
         col = c("black", "red"),
         lty = c(1, 2),
         lwd = c(2, 2),
         bty = "n",
...
plot_intervals <- function(result) {
  ...
  legend("topright",
         legend = c("Cubre", "No cubre", expression(mu)),
...
B <- 100        # número de intervalos a simular (menos para visualizar mejor)
